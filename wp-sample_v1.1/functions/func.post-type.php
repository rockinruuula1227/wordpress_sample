  ...
  public function add_custom_column_id($column_name, $id) {
    ...
    foreach ($this->pages as $page) {
      ...
      elseif ($column_name === $thumb_field):
        ...
        if ($thumb_ids === NULL) { echo '—'; continue;}
        if (!isset($thumb_ids[0])) { echo '—'; continue;}
        if ($thumb_ids[0] === '') { echo '—'; continue;}

    		$thumb =	wp_get_attachment_image($thumb_ids[0], array(100,100), 'thumbnail');
        echo ($thumb && $thumb !== '') ? $thumb : '—';
      endif;
    }
  }
